...
//	mpz_powm((*pub)->h_s, (*pub)->h, (*pub)->n, (*pub)->n_squared);
    ...
    mpz_inits((*prv)->lambda1, (*prv)->lambda2, NULL);
    aby_prng((*prv)->lambda1, mpz_sizeinbase((*pub)->n, 2));
    mpz_sub((*prv)->lambda2, (*prv)->lambda, (*prv)->lambda1);
...
void djn_decrypt_shared_1(mpz_t res, djn_pubkey_t* pub, djn_prvkey_t* prv, mpz_t ct)
{
    djn_pow_mod_n_squared_crt(res, ct, prv->lambda1, pub, prv);
}

void djn_decrypt_shared_2(mpz_t res, djn_pubkey_t* pub, djn_prvkey_t* prv, mpz_t ct, mpz_t ct1)
{
    djn_pow_mod_n_squared_crt(res, ct, prv->lambda2, pub, prv);
...
void djn_hm_scalarmult(djn_pubkey_t* pub, mpz_t res, mpz_t ct, mpz_t exp)
{
    mpz_powm(res, ct, exp, pub->n_squared);
}

void djn_hm_add(djn_pubkey_t* pub, mpz_t res, mpz_t ct_1, mpz_t ct_2)
{
    //gmp_printf("HM ADD: %Zd, %Zd, %Zd\n", ct_1, ct_2, pub->n_squared);
    mpz_mul(res, ct_1, ct_2);
    mpz_mod(res, res, pub->n_squared);
}
